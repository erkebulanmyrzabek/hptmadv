  ...
  methods: {
    async login(initData) {
      ...
      try {
        ...
        console.log('Sending to:', `${import.meta.env.VITE_BACKEND_API_URL}/api/users/telegram-auth/`);
        ...
        this.errorMessage = 'Не удалось авторизоваться: ' + (error.response?.data?.detail || error.message);
      } finally {
        this.loading = false;
      }
    },
    ...
    async loginWithRealUser() {
      if (!window.Telegram || !window.Telegram.WebApp) {
        this.errorMessage = 'Telegram WebApp не инициализирован';
        return;
      }
      ...
      if (!initData) {
        this.errorMessage = 'Telegram initData не доступен';
        return;
      }
